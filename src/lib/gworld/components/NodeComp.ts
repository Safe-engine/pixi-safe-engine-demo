...
type TouchEVentCallback = (target: { location: Vec2 | Point }) => void
...
export class NodeComp {
  ...
  get position(): Point | Vec2 {
    return this.getPosition()
  }

  set position(val: Point | Vec2) {
  ...
  convertToNodeSpace(point: Point | Vec2) {
    return this.instance.toLocal(point)
  }

  convertToNodeSpaceAR(point: Point | Vec2) {
    return this.instance.toLocal(point)
  }

  convertToWorldSpaceAR(point: Point | Vec2) {
    return this.instance.toGlobal(point)
  }

  getPosition(): Point | Vec2 {
    return this.instance.position
  }

  setPosition(x: number | Point | Vec2, y?: number) {
    if (typeof x !== 'number') {
      this.x = x.x
      this.y = x.y
    } else {
